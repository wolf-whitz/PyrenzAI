  ...
  return (
    <Box
      className={`w-full min-h-[360px] rounded-lg shadow-md border border-gray-600 bg-gray-800 mb-4 overflow-hidden
        ${isLoaded ? 'opacity-100 scale-100' : 'opacity-0 scale-95'} transition-opacity transition-transform duration-700`}
    >
      <Box
      ...
      <Box sx={{ p: 3 }}>
        <Box
          className={`h-6 bg-gray-500 w-3/4 mb-2 rounded
            ${isLoaded ? 'w-3/4' : 'w-0'} transition-width delay-400 duration-500`}
        ></Box>

        <Box
          className={`h-4 bg-gray-500 w-full mb-2 rounded
            ${isLoaded ? 'w-full' : 'w-0'} transition-width delay-600 duration-500`}
        ></Box>

        <Box
          className={`h-4 bg-gray-500 w-2/3 mb-2 rounded
            ${isLoaded ? 'w-2/3' : 'w-0'} transition-width delay-800 duration-500`}
        ></Box>
        ...
        >
          <Box
            className={`h-5 w-12 bg-gray-500 rounded
              ${isLoaded ? 'w-12' : 'w-0'} transition-width delay-[1.6s] duration-500`}
          ></Box>
          <Box
